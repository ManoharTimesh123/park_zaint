			...
			<form class="row g-3" action="{{ route('admin.booking.update', $booking->id) }}" method="post" name="booking_form">
				...
				<div class="g-3 row justify-content-center">
					...
					<div class="col-6 d-flex">
						...
						<input type="text" class="shadow-none form-control @error('email') is-invalid @enderror" name="email" id="email" placeholder="Email" value="{{ $user['email'] }}" />
						@error('email')
						<span class="invalid-feedback" role="alert">
							<strong>{{ $message }}</strong>
						</span>
					...
					<div class="col-6 d-flex">
						...
						<select id="airport" name="airport" class="form-select
								@error('airport') is-invalid @enderror" disabled>
							<option value="" selected>Select airport</option>
							@foreach ($airports as $airport)
							<option value="{{ $airport->id }}" {{ $booking->airport_id == $airport->id ? 'selected="selected"' : '' }}>
								{{ $airport->name }}
							</option>
							@endforeach
						</select>
					...
					<div class="col-6 d-flex">
						...
						<select id="product_id" name="product_id" class="form-select @error('product_id') is-invalid @enderror" disabled>
						@foreach ($products as $product)
							<option value="{{ $product->id }}" 
								@foreach ($booking->productAndAddon as $product_id)
									{{ $product_id->product_id == $product->id ? 'selected="selected"' : '' }}
								@endforeach
							>
								{{ $product->name }}
							</option>
					...
					<div class="col-6 d-flex">
						...
						<select id="inbound_terminal" name="inbound_terminal" class="form-select
								...
								@foreach ($terminals['airportterminals'] as $terminal)
									<option value="{{ $terminal->id }}" {{ $terminal->id == $booking->inbound_terminal_id ? 'selected' : '' }}>{{$terminal->name}}</option>
								@endforeach
					...
					<div class="col-6 d-flex">
						...
						<select id="outbound_terminal" name="outbound_terminal" class="form-select
							...
							@foreach ($terminals['airportterminals'] as $terminal)
								...
								<option value="{{ $terminal->id }}" {{ $terminal->id == $booking->outbound_terminal_id ? 'selected' : '' }}>{{$terminal->name}}</option>
							@endforeach
						</select>
					...
					<div class="col-6 d-flex">
						...
						<input type="date" id="datepicker" name="travel_start_date" class="shadow-none form-control @error('travel_start_date') is-invalid @enderror" value="{{ $booking->travel_start_date }}" min="{{ date('Y-m-d') }}" disabled>
					</div>

					<div class="col-6 d-flex">
						<label for="datepicker" class="form-label text-dark col-6">Travel end date:</label>
						<input type="date" id="datepicker" name="travel_end_date" class="shadow-none form-control @error('travel_end_date') is-invalid @enderror" value="{{ $booking->travel_end_date }}" min="{{ date('Y-m-d') }}" disabled>
					</div>
